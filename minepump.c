...
void WaterLevelMonitoring_Body(void) {
  ...
  if(ReadHLS() == 1){
    level = HighLevel;
    MI_write(LvlWater,level); // mutex 1 lock and unlock
  }
  else if(ReadLLS() == 0){
    level = LowLevel;
    MI_write(LvlWater,level); // mutex 1 lock and unlock
...
void MethaneMonitoring_Body (void) {
  ...
  MI_write(LvlMeth,level); // mutex 2 
  sem_post(&synchro);
...
/* PumpCtrl_Task is a sporadic task, it is triggered by a synchronization
   ...
   - if the alarm level is different from Normal, it BYTEsends the value 1
...
void *PumpCtrl_Body(void *no_sem_initargument) {
  int niveau_eau, niveau_alarme, alarme;
  int cmd=0;

   niveau_alarme = MI_read(LvlMeth); // mutex 2
   niveau_eau = MI_read(LvlWater); // mutex 1
  ...
  for (;;) {    
   sem_wait(&synchro);
    ...
    msg_box_send(mbox_alarm, (char*)&alarme); // mutex 3
    CmdPump(cmd);	 
  }
}
...
void *CmdAlarm_Body() {
  int value;
  for (;;) {
    msg_box_receive(mbox_alarm,(char*)&value); // mutex 3
    CmdAlarm(value);
  }
}
...
int main(void) {
  ...
  waterPeriod.tv_nsec = 250 * 1000 * 1000;
  waterPeriod.tv_sec  = 0 ;
  create_periodic_task(waterPeriod, &WaterLevelMonitoring_Body);

  /* Create task MethaneMonitoring_Task */
  struct timespec methanePeriod;
  methanePeriod.tv_nsec = 100 * 1000 * 1000;
  methanePeriod.tv_sec  = 0 ;
  create_periodic_task(methanePeriod, &MethaneMonitoring_Body);
